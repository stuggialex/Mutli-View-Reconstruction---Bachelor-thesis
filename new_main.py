...
dict = {"iteration_number": ITERATION_NUM,
        ...
        "sigma": SIGMA,
        "gamma": GAMMA}
dict_as_str = dict_to_string(dict)
...
def loop():
    ...
    for x in range(ITERATION_NUM):
        ...
        rays_o, rays_d = get_rays_tensor_torch(IMAGE_HEIGHT, IMAGE_WIDTH, intrinsic[0][0], extrinsic)
        origin = rays_o[0][0][0]

        masked_output = apply_mask(dmap_of_chosen_camera, mask_of_chosen_camera)
        ...
        #shape: nx3
        unsqueezed_rays_d = transpose_rays_tensor(rays_d)
        ...
        srdf_torch_rays_o, srdf_torch_rays_d = get_rays_tensor_torch(IMAGE_HEIGHT, IMAGE_WIDTH, srdf_intrinsic[0][0][0], srdf_extrinsic)
        ...
        srdf_masked_output = apply_mask_tensor(srdf_dmap, srdf_mask)
        ...
        adam.step()
        print("--- Iteration runtime: %s seconds ---" % (time.time() - start_time))
    ...
    #     if idx == 0:
    #         tensor_difference = append_tensor(is_zero)
    #     else:
    #         tensor_difference = append_tensor(tensor_difference, is_zero)

    save_into_file(dmaps, CAMERAS, name="_result", variable_list=dict_as_str, bool=False)



loop()
print("--- Code runtime: %s seconds ---" % (time.time() - start_time_overall))
